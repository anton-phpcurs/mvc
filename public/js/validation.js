/* add and modified by sebastian at 30.07.2015 */
$(document).ready(function(){
    
    /*  это для плагина "masked input" можно впринципе задать и другой формат телефона
        но тогда не забыть сменить регулярки в методах Валидейт ниже*/
    $("#userTel").inputmask("+380(99)999-99-99");

    /*  добавил свой метод, который позволяет просто
        подставить регулярку, и по ней фильтровать
        поле. Как пользоваться, видно ниже */
    $.validator.addMethod("regex", function(value, element, regexpr) {          
        if (value != "") return regexpr.test(value);
        else return true;
    }, "Поле не соответствует формату ввода");  

    /*  методы, которые сравнивают значение поля с другим полем.
        Нужен в данном случае для сравнения ввода минимальной цены с максимальной и наоборот.
        Стандартная функция jqueryValidate->max к сожалению не даёт такой возможности */
    $.validator.addMethod("lessThen", function(value, element, param) {
        var target = $( param );
        if (target.val() != "") return value < target.val();
        else return true;
    }, "Поле должно быть меньше максимального значения");
    $.validator.addMethod("moreThen", function(value, element, param) {
        var target = $( param );
        if (target.val() != "") return value > target.val();
        else return true;
    }, "Поле должно быть больше минимального значения");

    //форма "войти" на главной в выпадающем меню
    $('#logInHeader').validate({
       rules:{
            email:{
                required: true,
                minlength: 5,
       ...
       messages:{
            email:{
                ...
                minlength: "Email должен быть<br>минимум 5 символов",
                maxlength: "Максимальное число<br>символов - 45",
                email:"Введите корректный<br>email",
            },
            password:{
                required: "Введите пароль!",
                minlength: "Пароль должен<br>содержать<br>минимум 6 символов",
                maxlength: "Пароль должен<br>содержать<br>максимум 16 символов",
                regex: "Пароль некорректен",
            },
       }
    });
    
    //форма "войти" на отдельной странице
    $('#logInPage').validate({
       rules:{
            email:{
                required: true,
    ...
    //форма "регистрации" в поп-ап окошке
    $('#registpationPopupForm').validate({
       rules:{
            userName:{
                required: true,
                minlength: 2,
            ...
            userTel:{
                required: true,
                regex: /^\+[0-9]{3}\([0-9]{2}\)[0-9]{3}-[0-9]{2}\-[0-9]{2}/,
            },
            userPassword:{
                required: true,
                minlength: 6,
       ...
       messages:{
            userName:{
                ...
                maxlength: "Максимальная длянна - 20 символов",
                characters: "Имя может содержать только буквы",
                regex:  "Имя может содержать буквы латинского или русского алфавита",
            },
            userSurname:{
                required: "Введите фамилию пользователя",
                minlength: "Минимальная длинна - 2 символа",
                maxlength: "Максимальная длянна - 30 символов",
                regex:  "Фамилия может содержать буквы латинского или русского алфавита",
            },
            userEmail:{
                required: "Введите email!",
                minlength: "Email должен быть минимум 5 символов",
            ...
            userTel:{
                required: "Введите номер телефона в указаном формате",
                regex: "Введите номер телефона в указаном формате",
            },
            userPassword:{
                required: "Введите пароль!",
                minlength: "Пароль должен содержать минимум 6 символов",
   ...
   //форма поиска товаров
    $('#searchGoodsForm').validate({
       ...
       messages:{
            searchText:{
                required: "Введите запрос перед поиском",
                minlength: "Минимальная длинна запроса - 2 символа",
            },
       }
    });

    //форма фильтрации товаров (ЗАГОТОВКА, НУЖНА ДОРАБОТКА)
    $('#filterForm').validate({
       rules:{
            /*category1Check:{
            },
            ...
            minPrice:{
                regex: /^[0-9\.]+$/,
                lessThen: '#filterForm input[name=maxPrice]',
            },
            maxPrice:{
                regex: /^[0-9\.]+$/,
                moreThen: '#filterForm input[name=minPrice]',    
            },
       },
       messages:{
            minPrice:{
                regex: "Вводите только цифры!",
                lessThen: "Цена больше максимальной",
            },
            maxPrice:{
                regex: "Вводите только цифры!",
                moreThen: "Цена меньше минимальной",    
            },
       }
    });

    //форма создания (добавления нового товара) goods/create
    $('#createProductUnit').validate({
        rules:{
            productName:{
                required: true,
            ...
            productAuctEndTime:{
                required: true,
                regex: /^[0-9]{2}-[0-9]{2}-[0-9]{4}\s[0-9]{2}:[0-9]{2}/,
            },
            productPhoto:{
                required: true,
            },
        ...
        messages:{
            productName:{
                ...
                regex: 'Поле может содержать символы русского и <br>латинского алфавитов, цифры,<br> знаки -, !, (, ) и пробелы',
                minlength: 'Название должно содержать более 2 символов',
                maxlength: 'Название не должно превышать 45 символов',
            },
            productDescription:{
                required: 'Введите описание товара',
                minlength: 'Описание должно содержать хотябы 15 символов',
            },
            productStartPrice:{
                required: 'Введите стартовую цену',
                regex: 'Цена не соответствует заданому формату',
            },
            productDeltaPrice:{
                required: 'Введите шаг цены',
                regex: 'Шаг цены не соответствует заданому формату',
            },
            productAuctEndTime:{
                required: 'Введите дату окончания торгов',
                regex: 'Дата не соответствует формату, <br>используйте выпадающий календарь',
            },
            productPhoto:{
                required: 'Загрузите фото товара',
            },
        }
    });
